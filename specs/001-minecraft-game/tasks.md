...
- [ ] T001 Create directory structure: `src/world/`, `src/player/`, `src/entities/`, `src/crafting/`, `src/ui/`, `src/persistence/`, `tests/unit/`, `tests/integration/`, `tests/contract/`, `saves/`, `assets/textures/`
- [ ] T002 Create `requirements.txt` with `ursina>=6.0.0`, `noise>=1.2.2`, `pytest>=7.0.0`, `pytest-mock>=3.0.0`
- [ ] T003 [P] Create `pytest.ini` at repository root configuring `testpaths = tests` and `python_files = test_*.py`
- [ ] T004 [P] Add `__init__.py` files to all `src/` packages and `tests/` sub-packages
...
- [ ] T005 Define `BlockType` enum (AIR, GRASS, DIRT, STONE, WOOD, LEAVES, WATER, SAND, ORE_COAL, ORE_IRON, CRAFTING_TABLE, CHEST) and `Block` properties dataclass (`hardness`, `drop`, `is_solid`, `texture`) in `src/world/block.py`
- [ ] T006 [P] Define `ItemType` enum (WOOD, STICK, STONE, COAL, IRON_ORE, IRON_INGOT, WOOD_PLANKS, WOOD_PICKAXE, STONE_PICKAXE, IRON_PICKAXE, WOOD_SWORD, STONE_SWORD, IRON_SWORD, TORCH, CRAFTING_TABLE) and `ItemStack` dataclass (`item_type`, `quantity`, `max_stack`) in `src/player/inventory.py`
- [ ] T007 [P] Define `EnemyType` enum (ZOMBIE, SKELETON) and `EnemyState` enum (IDLE, ALERT, CHASE, ATTACK, DEAD) in `src/entities/enemy.py`
...
- [ ] T008 [P] [US1] Write unit tests for `Chunk` (block get/set, sparse storage, dirty flag) in `tests/unit/test_world.py`
- [ ] T009 [P] [US1] Write unit tests for `World` (procedural gen seed determinism, `get_block`, `set_block`, `break_block`, `place_block`) in `tests/unit/test_world.py`

### Implementation for User Story 1

- [ ] T010 [US1] Implement `Chunk` class (`position`, `blocks` dict, `is_dirty`, `is_loaded`, `get_block`, `set_block`) in `src/world/chunk.py`
- [ ] T011 [US1] Implement `World` class with Perlin-noise procedural generation (`__init__`, `get_block`, `set_block`, `break_block`, `place_block`, `get_chunk`, `load_chunks_around`, `update`) in `src/world/world.py`
- [ ] T012 [US1] Implement `Player` controller with movement (WASD), jump (Space), mouse-look, `break_targeted_block`, `place_block`, `take_damage`, `heal`, `is_alive`, `respawn` in `src/player/player.py`
- [ ] T013 [US1] Create main entry point: initialise Ursina app, create `World` and `Player`, wire input handlers and game loop in `src/main.py`
...
- [ ] T014 [P] [US2] Write unit tests for `Inventory` (`add_item`, `remove_item`, `has_items`, `get_hotbar`, `get_selected_item`, `to_dict`, `from_dict`, full-inventory rejection) in `tests/unit/test_inventory.py`
- [ ] T015 [P] [US2] Write unit tests for `RecipeRegistry` and `craft` function (valid recipe, insufficient materials, unknown recipe) in `tests/unit/test_crafting.py`

### Implementation for User Story 2

- [ ] T016 [US2] Implement `Inventory` class (slots list, `add_item`, `remove_item`, `has_items`, `get_hotbar`, `get_selected_item`, `to_dict`, `from_dict`) per game-api.md contract in `src/player/inventory.py`
- [ ] T017 [P] [US2] Implement `ItemDrop` entity (visual entity, auto-pickup on player proximity, despawn after collection) in `src/entities/item_drop.py`
- [ ] T018 [P] [US2] Implement `RecipeRegistry` and `craft` function with at least 10 recipes (wood planks, sticks, pickaxes, swords, torches, crafting table) per game-api.md contract in `src/crafting/recipe.py`
- [ ] T019 [US2] Implement `CraftingUI` (3×3 grid input, recipe matching display, craft button, close on `E`) in `src/crafting/crafting_ui.py`
- [ ] T020 [US2] Implement HUD hotbar (9-slot display, selected-slot highlight, scroll/number-key slot cycling) in `src/ui/hud.py`
- [ ] T021 [US2] Wire item drops into `World.break_block` (spawn `ItemDrop` at block position) and integrate `Inventory` into `Player` in `src/world/world.py` and `src/player/player.py`
...
- [ ] T022 [P] [US3] Write unit tests for `Player` health system (`take_damage`, `heal`, death condition, `is_alive`, `respawn`) in `tests/unit/test_health.py`
- [ ] T023 [P] [US3] Write unit tests for `Enemy` state-machine transitions (IDLE→ALERT→CHASE→ATTACK→DEAD, night/day toggling) in `tests/unit/test_enemy.py`

### Implementation for User Story 3

- [ ] T024 [US3] Implement health module helpers (`take_damage`, `heal`, clamp to 0–max_health) used by `Player` in `src/player/health.py`
- [ ] T025 [US3] Implement day/night cycle in `World.update` (`day_time` advances with `time_speed`, configurable full-cycle duration ~20 min) in `src/world/world.py`
- [ ] T026 [US3] Implement `Enemy` class (state machine per data-model.md: IDLE/ALERT/CHASE/ATTACK/DEAD, `update`, `take_damage` with loot drops, `is_alive`, `get_attack_damage`) per game-api.md contract in `src/entities/enemy.py`
- [ ] T027 [US3] Add HUD health bar (current/max health display, damage flash indicator) to `src/ui/hud.py`
- [ ] T028 [US3] Wire enemy spawning at night, player attack (left-click on enemy), damage exchange, death/respawn flow, and world lighting change into `src/main.py`
...
- [ ] T029 [P] [US4] Write unit tests for block placement validation in `World.place_block` (occupied-space rejection, valid adjacent placement, multiple consecutive placements) in `tests/unit/test_world.py`

### Implementation for User Story 4

- [ ] T030 [US4] Ensure `Chunk.is_dirty` triggers mesh rebuild when any block is placed or removed in `src/world/chunk.py`
- [ ] T031 [US4] Validate `World.place_block` rejects occupied positions and returns `False`; apply sparse block dict update in `src/world/world.py`
- [ ] T032 [US4] Add block-type selection via hotbar cycling (scroll wheel, 1–9 keys) and show selected block type on HUD in `src/ui/hud.py`
- [ ] T033 [US4] Ensure at least 5 distinct block textures are referenced in `Block` properties (grass, dirt, stone, wood, leaves) in `src/world/block.py`
...
- [ ] T034 [P] [US5] Write contract test verifying `metadata.json` and `world.pkl` schema match save-format.md spec in `tests/contract/test_save_format.py`
- [ ] T035 [P] [US5] Write unit tests for `SaveManager` (`save`, `load`, `list_saves`, `delete_save`, round-trip integrity) in `tests/unit/test_save_manager.py`

### Implementation for User Story 5

- [ ] T036 [US5] Implement `World.to_save_data` / `World.from_save_data` serialising only modified chunks to the `WorldSave` schema in `src/world/world.py`
- [ ] T037 [US5] Implement `SaveManager` (`save` writes `metadata.json` + `world.pkl`, `load` returns `WorldSave`, `list_saves`, `delete_save`, `auto_save`) per game-api.md contract and save-format.md in `src/persistence/save_manager.py`
- [ ] T038 [US5] Implement main menu (`New Game`, `Load Game` list, `Quit`) using Ursina UI in `src/ui/main_menu.py`
- [ ] T039 [US5] Wire `F5` manual save, save-on-quit confirmation, and 5-minute auto-save timer into `src/main.py`
...
- [ ] T040 [P] Create `config.json` at repository root with configurable settings (`auto_save_interval_seconds: 300`, `chunk_render_radius: 4`, `day_cycle_seconds: 1200`)
- [ ] T041 [P] Add `saves/` and `.venv/` to `.gitignore`
- [ ] T042 Write integration test covering full gameplay flow (world gen → move → break → craft → save → load) in `tests/integration/test_gameplay.py`
- [ ] T043 [P] Audit type hints and add missing annotations across all `src/` modules
